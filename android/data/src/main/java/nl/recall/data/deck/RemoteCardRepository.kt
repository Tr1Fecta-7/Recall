...
class RemoteCardRepository(private val cardDao: CardDao) : CardRepository {
    override suspend fun resetAlgorithm(deckId: Long): Boolean {
        val response = cardDao.resetAlgorithm(deckId, Date())
    ...
    override suspend fun getCardById(deckId: Long, cardId: Long): Card {
        return cardDao.getCardById(deckId, cardId)?.toDomain()
    ...
    override suspend fun saveCard(
        ...
        val cardEntityRow: Long = cardDao.insert(
            CardEntity(
                id = 0,
                front = front,
                back = back,
    ...
    override suspend fun updateCard(card: Card): Boolean {
        return cardDao.updateCard(
            CardEntity(
                id = card.id, front = card.front, back = card.back,
                dueDate = card.dueDate, deckId = card.deckId, successStreak = card.successStreak
            )
        ) >= 0
    }
